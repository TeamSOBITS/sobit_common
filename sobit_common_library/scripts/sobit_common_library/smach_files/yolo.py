...
def yolo_predict_ctrl(mode):
    """
    yoloによる推論の実行をコントロールするサービスを呼び出す
    Args:
        mode          : True = 推論の開始, False = 推論のストップ
    Returns:
        True or False : 成功ならTrue, 失敗ならFalse
    """
    rospy.wait_for_service("/pytorch_yolo/run_ctrl", 3.0)
    try:
        service = rospy.ServiceProxy("/pytorch_yolo/run_ctrl", RunCtrl)
        res = service(mode)
        return res.response
...
def start_object_recognition(target_obj, timeout_sec=15.0):
    ...
    Args:
        ...
        timeout_sec   : Yoloによる推論の最大起動時間[sec] 
    Returns:
    ...
    yolo_predict_ctrl(True)
    rospy.Subscriber("/pytorch_yolo/detect_list", StringArray, callback_detect_objects)
